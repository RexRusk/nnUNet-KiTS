...
class MultiHead_Module(nn.Module):
    ...
    def __init__(self, class_object: Type[nn.Module], split_at, task, prev_trainer=None, *args, **kwargs):
        ...
        if prev_trainer is None:
            ...
            assert isinstance(prev_trainer, self.class_object), \
                "This function splits a \'{}\' module class object, but a \'{}\' module is provided.".format(
                    self.class_object.__name__, type(prev_trainer))
        ...
        while simplify:
            ...
            if self.split[-1] == first_layer_name:
                ...
                # -- Print a Note -- #
                print(
                    'Note: The split \'{}\' has been transformed to \'{}\' since it specifies the same split.\n'.format(
                        '.'.join(self.split), '.'.join(new_split)))
                # -- Update self.split
            ...
            assert not (
                        len(self.split) == 1 and simplify), "The provided split \'{}\' is empty after simplification and would split before the first layer..".format(
                '.'.join(split))
        ...
        assert isinstance(task, (str, int)), "The provided task needs to be an integer (ID) or string, not {}..".format(
            type(task))
        ...
        self.body, init_module, _, _ = self._split_model_recursively_into_body_head(layer_id=0,
                                                                                    model=self.model)  # Start from root with full model
    ...
    def forward(self, x):
        ...
        res = self.class_object.forward(self.model,
                                        x)  # --> Do not use super, since we want to set the correct self object ;)
    ...
    def update_after_iteration(self, model=None, update_body=True):
        ...
        if update_body:  # Update body and head both
            self.body, head, _, _ = self._split_model_recursively_into_body_head(layer_id=0,
                                                                                 model=model)  # Start from root with full model
        else:  # Do not update the body, ie. only update the head
            _, head, _, _ = self._split_model_recursively_into_body_head(layer_id=0,
                                                                         model=model)  # Start from root with full model
        ...
        for idx, (name, n_module) in enumerate(list(model.named_children())):
            ...
            # -- Split is not reached yet -- #
                ...
                if name == self.split[len(parent)]:
                    ...
                    body, head, layer_id, parent = self._split_model_recursively_into_body_head(layer_id + 1, n_module,
                                                                                                body, head, parent)
    ...
    def assemble_model(self, task, freeze_body=False):
        ...
        assert task in self.heads.keys(), \
            "The provided task \'{}\' is not a known head, so either initialize the task or provide one that already exists: {}.".format(
                task, list(self.heads.keys()))
